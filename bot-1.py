...
async def get_all_matches():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # TheSportsDB as fallback if no basketball found yet
            ...
            for date_str in dates:
                ...
                for ev in events[:20]:
                    try:
                        ...
                        if event_id in seen_bball_ids:
                            continue
                        seen_bball_ids.add(event_id)
                        pred = predict_basketball([], [])
                        match = format_basketball_event(ev, pred)
                        basketball_matches.append(match)
                    except Exception as e:
                        logger.warning(f"SportsDB basketball error: {e}")
                        continue
