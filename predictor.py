...
class MatchPredictor:
    ...
    async def build_daily_ticket(self, manual_picks: list = None):
        ...
        for r in results:
            if isinstance(r, dict) and r.get("confidence", 0) >= 40:
                analyzed.append(r)
    ...
    async def deep_analyze(self, fixture_id: int) -> dict:
        ...
        # Gather all data with small delays to respect free plan rate limit (10 req/min)
        pred = await self.api.get_fixture_predictions(fixture_id)
        await asyncio.sleep(1)
        h2h = await self.api.get_head_to_head(home["id"], away["id"], last=10)
        await asyncio.sleep(1)
        home_form = await self.api.get_team_last_matches(home["id"], last=6)
        await asyncio.sleep(1)
        away_form = await self.api.get_team_last_matches(away["id"], last=6)
        await asyncio.sleep(1)
        odds_data = await self.api.get_fixture_odds(fixture_id)
        injuries = await self.api.get_injuries(fixture_id)
