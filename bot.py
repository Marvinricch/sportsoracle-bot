...
async def get_all_matches():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # ── Basketball via TheSportsDB ─────────────────────────────────────
        for date_str in dates:
            events = await fetch_basketball_events(session, date_str)
            for ev in events[:30]:
                try:
                    home_id = ev.get("idHomeTeam")
                    away_id = ev.get("idAwayTeam")
                    home_last5, away_last5 = await asyncio.gather(
                        fetch_team_last5(session, home_id),
                        fetch_team_last5(session, away_id),
                    )
                    pred = predict_basketball(home_last5, away_last5)
                    match = format_basketball_event(ev, pred)
                    basketball_matches.append(match)
                except Exception as e:
                    logger.warning(f"Basketball event error: {e}")
                    continue
